...
Do not submit unless every required item is checked.
...
- [x] Neutral-content non-intervention is not claimed; activation is explicitly user-controlled.

## Security and release

- [x] Secret/PII scan finds no real credential, token, email, phone, account name, or private URL; the one email-shaped value is an explicit synthetic `example` test fixture.
- [ ] Git diff contains only project files.
- [ ] No `.env`, terminal history, raw HTTP trace, provider body, or screenshot metadata is published.
- [x] No external code, model, wheel, native binary, dynamic import, or subprocess is used by the application or evidence Notebook.
- [x] Application code license and third-party scripture/provider terms are distinguished.
- [ ] Public links are copied into the final Writeup and rechecked.
